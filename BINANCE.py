...
while True:
    ...
    def moshort(loaicoin,soluong):
        ...
        tp = df['Close'].iloc[-1] - 1.5 * atr  # SL = 1.5x ATR below entry
        sl = df['Close'].iloc[-1] + 2 * atr    # TP = 2x ATR above entry
    ...
    def molong(loaicoin,soluong):
        ...
        atr = ta.atr(df['High'], df['Low'], df['Close'], length=14).iloc[-1]
        sl = df['Close'].iloc[-1] - 1.5 * atr  # SL = 1.5x ATR below entry
        tp = df['Close'].iloc[-1] + 2 * atr    # TP = 2x ATR above entry
    ...
    if env.balance > 1000 :
        ...
        if   vithedangmo  == False and action == 0 and  (df['xuhuongmacd'].iloc[-1]) > 0  and df['Close'].iloc[-1] - df['Open'].iloc[-1]  > 0 and    df2['Close'].iloc[-1] - df2['Open'].iloc[-1]  > 0 :
